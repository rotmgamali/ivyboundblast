    ...
    def __init__(self, headless: bool = True, sheet_name: str = None):
        ...
        try:
            ...
            for record in all_records:
                ...
                if bname: self.seen_leads.add(str(bname).lower().strip())
                
                phone = record.get('phone')
                if phone: self.seen_leads.add(str(phone).strip())
                
                domain = record.get('domain')
                if domain: self.seen_leads.add(self._clean_domain(str(domain)))
